...
function getTopSellerGames() {
    ...
    // Lấy các game có tag là 'topsellers', giới hạn tối đa 10 bản ghi
    $sql = "SELECT gameid, gamename, price, sale, tag FROM game WHERE tag = 'topsellers' || tag = 'mostplayed' LIMIT 10";
    $result = $conn->query($sql);

    // Mảng để lưu trữ dữ liệu game
    $gameList = array();

    // Lấy dữ liệu từ kết quả trả về sử dụng fetch()
    while($row = $result->fetch(PDO::FETCH_ASSOC)) {
        // Thêm dữ liệu của mỗi game vào mảng
        $gameList[] = $row;
    }

    // Trả về mảng chứa dữ liệu game
    return $gameList;
}
